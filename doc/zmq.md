...
The BitcoinReloaded Core daemon can be configured to act as a trusted "border
router", implementing the bitcoinreloaded wire protocol and relay, making
...
The ZeroMQ feature in BitcoinReloaded Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitcoinreloadedd:
...
For instance:
    ...
    $ bitcoinreloadedd -zmqpubhashtx=tcp://127.0.0.1:211267 \
               -zmqpubhashtx=tcp://192.168.1.2:211267 \
               -zmqpubrawtx=ipc:///tmp/bitcoinreloadedd.tx.raw \
...
These options can also be provided in bitcoinreloaded.conf.
...
From the perspective of bitcoinreloadedd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitcoinreloadedd directly. Furthermore, no information is
...
using. BitcoinReloadedd appends an up-counting sequence number to each
