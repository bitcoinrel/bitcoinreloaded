    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return BitcoinReloadedUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, BitcoinReloadedUnits::SeparatorStyle::NEVER);
            else
                return BitcoinReloadedUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+BitcoinReloadedUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
