...
###   bitcoinreloadedcore.org and bitcoinreloaded.org and compares them.
...
WORKINGDIR="/tmp/bitcoinreloaded_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitcoinreloadedcore.org"
HOST2="https://bitcoinreloaded.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitcoinreloaded-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoinreloaded-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitcoinreloaded.org failed to provide signature file, but bitcoinreloadedcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoinreloaded.org and bitcoinreloadedcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BitcoinReloaded Core binary release signing key installed?"
   fi
