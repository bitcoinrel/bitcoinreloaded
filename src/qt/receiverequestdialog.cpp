...
#include <config/bitcoinreloaded-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatBitcoinReloadedURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(BitcoinReloadedUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatBitcoinReloadedURI(info));
}
