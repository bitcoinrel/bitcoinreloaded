...
const char * const BITCOINRELOADED_CONF_FILENAME = "bitcoinreloaded.conf";
const char * const BITCOINRELOADED_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/bitcoinreloaded/bitcoinreloaded/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinreloaded-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BITCOINRELOADED_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "bitcoinreloaded";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinReloaded
    // macOS: ~/Library/Application Support/BitcoinReloaded
    // Unix-like: ~/.bitcoinreloaded
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinReloaded";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinReloaded";
#else
    // Unix-like
    return pathRet / ".bitcoinreloaded";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BITCOINRELOADED_CONF_FILENAME);
