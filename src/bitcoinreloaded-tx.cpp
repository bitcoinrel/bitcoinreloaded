...
static void SetupBitcoinReloadedTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoinReloadedTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinreloaded-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitcoinreloaded-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinreloaded transaction\n" +
            "or:     bitcoinreloaded-tx [options] -create [commands]   Create hex-encoded bitcoinreloaded transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinreloaded transaction
